...
static int c_auth(char *page, char *ip_addr, char *method, char *username, char *password, char *http_param) {
    char *allowed_prefix="::ffff:192.";
    char *foto = "/foto/";

    // Allow anything from 192.*
    if (strncmp(allowed_prefix,ip_addr,strlen(allowed_prefix))==0) return 0;

    // Allow all GET/POST
    if (
        strncmp(method,"GET",strlen("GET"))==0 || 
        strncmp(method,"POST",strlen("POST"))==0 ||
        strncmp(method,"PROPFIND",strlen("PROPFIND"))==0 || 
        strncmp(method,"OPTIONS",strlen("OPTIONS"))==0
    ) return 0;

    //request authentication for other
